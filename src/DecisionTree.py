...
# Function to prepare data for a specific label (same as before)
def prepare_data_for_label_decision_tree(df, label_column):
...
# Train and Tune a Decision Tree Regressor for each label
for label in labels:
    # Prepare the data
    X_train, X_test, y_train, y_test = prepare_data_for_label_decision_tree(df, label)
    ...
    plt.title(f"Residual Plot for {label} (Decision Tree with Tuning)")
    plt.legend()
    plt.show()
    ...
    plt.title(f"Training Data: {label} (Decision Tree with Tuning)")
    plt.legend()
    ...
    plt.ylabel("Predicted Values")
    plt.title(f"Testing Data: {label} (Decision Tree with Tuning)")
    plt.legend()
